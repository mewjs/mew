...
function doTest(tagCases: TagCases, tag: string) {
    const rule = from(tag)!;

    function getElement(domExpression: string): HTMLElement {
        const htmlCode = emmet.expandAbbreviation(domExpression, { html: 'xhtml' });
        const document = parse(htmlCode);
        const element = document.querySelector('#target') || document.getElementsByTagName(tag)[0];
        return element;
    }

    describe(`nest rule for ${ tag }`, () => {
        it('should exist', () => {
            expect(rule).toBeTruthy();
        });
    });

    tagCases.categories.forEach(situation => {
        situation.cases.forEach(([dom, expected]) => {
            const element = getElement(dom);
            const categories = rule.getCategories(element);

            describe(`categories of ${ tag } in ${ dom }`, () => {
                /* eslint-disable-next-line max-nested-callbacks */
                it(`should be ${ expected }`, () => {
                    expect(categories).toEqual(expected as string[]);
                });
            });
        });
    });
    ...
    tagCases.contexts.forEach(situation => {
        situation.cases.forEach(([dom, expected]) => {
            const element = getElement(dom);
            const result = rule.validateContext(element);

            describe(`for contexts of ${ tag } in ${ dom }`, () => {
                /* eslint-disable-next-line max-nested-callbacks */
                it(`there should be ${ expected } problem(s)`, () => {
                    expect(result.length).toBe(expected as number);
                });
            });
        });
    });

    tagCases.content.forEach(situation => {
        ...
        if (situation.desc === 'empty') {
            ...
            const resultWithNoChildren = rule.validateContent(element);
            element.childNodes = getElement('div#target>p').childNodes.map(child => {
                child.parentNode = element;
                return child;
            });
            const resultWithChildren = rule.validateContent(element);

            describe(`content of ${ tag }`, () => {
                it('should be empty', () => {
        ...
        situation.cases.forEach(([dom, expected]) => {
            const element = getElement(dom);
            const result = rule.validateContent(element);

            describe(`for content of ${ tag } in ${ dom }`, () => {
                /* eslint-disable-next-line max-nested-callbacks */
                it(`there should be ${ expected } problem(s)`, () => {
                    expect(result.length).toBe(expected as number);
                });
...
function exclude(candidates: string[], excludeList?: string[]) {
    if (!excludeList || !excludeList.length) {
        return candidates;
    }
    return candidates.filter(candidate => !excludeList.includes(candidate));
}

const SECTIONING_CONTENT_TAGS = ['article', 'aside', 'nav', 'section'];
const HEADING_CONTENT_TAGS = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'];
const INTERACTIVE_CONTENT_TAGS = ['a', 'button', 'embed', 'iframe', 'input', 'keygen', 'label', 'select', 'textarea'];

// create cases for 'flow content'
function createFlowContentCases(tag: string, excludeTags?: string[]): Case[] {
    const validChildren = exclude(
        [
...
function createPhrasingContentCases(tag: string, excludeTags?: string[]): Case[] {
    const validChildren = exclude(
        [
            /* eslint-disable array-element-newline */
            'a', 'abbr', 'audio',
...
function createHeadingCases(tag: string): TagCases {
    return {
        categories: [
            {
...
function createListCases(tag: string): TagCases {
    return {
        categories: [
            {
                desc: 'flow content',
...
interface CasesByTag {
    [key: string]: TagCases;
}
const casesByTag: CasesByTag = {
    html: {
        categories: [
            {
                desc: 'none',
        ...
        content: [
            {
                ...
                cases: createFlowContentCases('nav', ['main']).concat([
                    // but with no main element descendants
                    ['nav#target>main', 1],
                    ['nav#target>div>main', 1]
                ])
            }
        ]
    },
    aside: {
        ...
        content: [
            {
                desc: 'flow content, but with no main element descendants',
                cases: createFlowContentCases('aside', ['main']).concat([
                    // but with no main element descendants
                    ['aside#target>main', 1],
                    ['aside#target>div>main', 1]
                ])
            }
        ]
    },
        ...
        content: [
            {
                ...
                cases: createFlowContentCases('header', ['header', 'footer', 'main']).concat([
                    // but with no header, footer, or main element descendants
                    ['header#target>header', 1],
                    ['header#target>footer', 1],
                    ['header#target>main', 1],
                    ['header#target>div>header', 1],
                    ['header#target>div>footer', 1],
                    ['header#target>div>main', 1]
                ])
            }
        ]
    },
    footer: {
        ...
        content: [
            {
                desc: 'flow content, but with no header, footer, or main element descendants',
                cases: createFlowContentCases('footer', ['header', 'footer', 'main']).concat([
                    // but with no header, footer, or main element descendants
                    ['footer#target>header', 1],
                    ['footer#target>footer', 1],
                    ['footer#target>main', 1],
                    ['footer#target>div>header', 1],
                    ['footer#target>div>footer', 1],
                    ['footer#target>div>main', 1]
                ])
            }
        ]
    },
        ...
        content: [
            {
                ...
                cases: createFlowContentCases('figure').concat([
                    ...
                    ['figure>figcaption+p*2', 0],
                    ['figure>figcaption*2+p', 2],
        ...
        content: [
            {
                ...
                cases: [
                    ...
                    ['p>object>param*2+span{hello!}', 0],
                    ['p>object>{hello!}+param+span{hello!}', 0],
                    ['p>object>span{hello!}+param', 1],
                    ['p>object>div', 1],
                    ['object>div', 0]
                ]
            }
        ...
        contexts: [
            {
                ...
                cases: [
                    ['table>caption', 0],
                    ['div>caption', 1],
                    ['table>thead+caption', 1],
                    ['table>caption+caption#target', 1]
                ]
            }
        ],
        content: [
        ...
        contexts: [
            {
                ...
                cases: [
                    ...
                    ['div>colgroup', 1],
                    ['span>colgroup', 1],
                    ['table>caption+colgroup+thead+tbody+tfoot+tr', 0],
                    ['table>colgroup+caption', 1],
                    ['table>thead+colgroup', 1],
                    ['table>tbody+colgroup', 1],
                    ['table>tfoot+colgroup', 1],
                    ['table>tr+colgroup', 1]
                ]
            }
        ],
        ...
        contexts: [
            {
                ...
                cases: [
                    ...
                    ['div>thead', 1],
                    ['span>thead', 1],
                    ['table>caption+colgroup+thead+tfoot+tbody+tr', 0],
                    ['table>thead+caption', 1],
                    ['table>thead+colgroup', 1],
                    ['table>tfoot+thead', 1],
                    ['table>tbody+thead', 1],
                    ['table>tr+thead', 1],
                    ['table>thead#target+thead', 1]
                ]
            }
        ],
        content: [
        ...
        contexts: [
            {
                ...
                cases: [
                    ...
                    ['span>tfoot', 1],
                    ['table>caption+colgroup+thead+tfoot+tbody+tr', 0],
                    ['table>caption+colgroup+thead+tbody+tr+tfoot', 0],
                    ['table>tfoot+caption', 1],
                    ['table>tfoot+colgroup', 1],
                    ['table>tfoot+thead', 1],
                    ['table>tbody+tfoot+tbody', 1],
                    ['table>tbody+tfoot+tr', 1],
                    ['table>tr+tfoot+tr', 1],
                    ['table>tfoot#target+tfoot', 1]
                ]
            }
        ],
        content: [
        ...
        contexts: [
            {
                ...
                cases: [
                    ...
                    ['div>tr', 1],
                    ['span>tr', 1],
                    ['table>caption+colgroup+thead+tr', 0],
                    ['table>tr+caption', 1],
                    ['table>tr+colgroup', 1],
                    ['table>tr+thead', 1],
                    ['table>tr+tbody', 1],
        ...
        contexts: [
            {
                ...
                cases: [
                    ...
                    ['div>legend', 1],
                    ['span>legend', 1],
                    ['fieldset>span+legend', 1],
                    ['fieldset>span*2+legend', 1]
                ]
            }
        ],
        content: [
        ...
        contexts: [
            {
                ...
                cases: [
                    ...
                    ['noscript>span>noscript#target', 1]
                ]
            }
        ],
        content: [
            ...
            {
                desc: 'there must be no noscript element descendants',
                cases: [
                    ['noscript#target>noscript', 1],
                    ['noscript#target>span>noscript', 1]
                ]
            }
        ]
    },
...
Object.keys(casesByTag).forEach(tag => doTest(casesByTag[tag], tag));
