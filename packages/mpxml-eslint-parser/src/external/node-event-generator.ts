...
import esquery, { type Selector } from 'esquery';
    ...
    switch (parsedSelector.type) {
        ...
        case 'matches':
            return parsedSelector.selectors.reduce(
                (sum: number, childSelector: Selector) =>
...
interface ESQueryParseError extends Error {
    offset?: number;
}
...
function tryParseSelector(rawSelector: string): Selector {
    try {
        ...
        const error = e as ESQueryParseError;
        if (typeof error.offset === 'number') {
            throw new Error(
                `Syntax error in selector "${ rawSelector }" at position ${ error.offset }: ${ error.message }`
            );
        }
