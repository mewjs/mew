/* eslint-disable no-negated-condition, max-lines, line-comment-position */
// @ts-nocheck
...
export default class Tokenizer {
    ...
    private readonly lintMode: boolean;
    ...
    constructor(
        {
            ...
            lintMode = false,
            decodeEntities = true,
        }: { xmlMode?: boolean; lintMode?: boolean; decodeEntities?: boolean },
    ...
    private stateText(c: number) {
        if (
            ...
            if (this.lintMode) {
                while (this.buffer.charCodeAt(this._index + 1) === CharCodes.Lt && this._index < this.buffer.length) {
                    this._index++;
                }
            }
    ...
    private stateInSpecialTag(c: number) {
        ...
        if ((c | 0x20) === this.currentSequence[this.sequenceIndex]) {
            ...
            if (this.currentSequence === Sequences.TitleEnd && !this.lintMode) {
    ...
    private isTagStartChar(c: number) {
        return (this.xmlMode || this.lintMode) ? !isEndOfTagSection(c) : isASCIIAlpha(c);
    ...
    private stateBeforeTagName(c: number) {
        if (c === CharCodes.ExclamationMark) {
            ...
            const lower = c | 0x20;
            this.sectionStart = this._index;
            if (!this.xmlMode && !this.lintMode && lower === Sequences.TitleEnd[2]) {
                this.startSpecial(Sequences.TitleEnd, 3);
            } else {
                this._state =
                    (this.lintMode || !this.xmlMode) && lower === Sequences.ScriptEnd[2]
